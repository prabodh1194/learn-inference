...
> overlap. Two mechanisms do the work: splitting the command buffer, and
...
MPSCommandBuffer is a queue; the CPU can commit chunk *n* (without waiting)
and immediately begin encoding chunk *n+1* into the same buffer, while the GPU
...
60% split depth, with the M3 Max gaining ~1.2% only at 30/50 depth and
...
CUDA graphs) gives you the *option* of overlap — it does not give you the
...
the readbacks cost 0.1–0.3% of runtime, *and* they pinned ~136 MB of host
