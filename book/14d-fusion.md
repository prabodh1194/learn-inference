...
> never leaves the chip and a dispatch never happens. The common instinct is
...
2. **A global read+write** — B's input would have round-tripped through
   global memory; now it stays in registers or threadgroup memory.
3. **A separate pass** — if A and B have the same grid, one launch can do both
   bodies, and the grid is scheduled once.

A 5376-wide hidden state, BF16, is ~10.5 KiB per row of activations. Every
...
dispatches eliminated per forward pass for the int8 fused path, and the fused
kernel is the default because the numbers are simply better on both M5 and M3.
...
and 58.9 MiB (864-class) of intermediate traffic. Why does the *conversion
...
The MLP's SwiGLU computes two 14336-wide projections and a gated product. The
book's L16 instinct is to keep the intermediate on-chip. h3.c takes it
further: the whole SwiGLU tile — 32 KiB of threadgroup memory — is staged
inside the kernel and the gated product is computed in registers, so the
14336-wide intermediate **never leaves the SM**. The F32 accumulate happens
...
Not every fusion wins. The GQA/MQA attention path was built as a cached
