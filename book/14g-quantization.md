...
> *discipline* around it: h3.c's int8 path doesn't claim "2× smaller and
...
The DiT's linear layers run in int8 with BF16 accumulation. The quantization
contract, in full:

- **Weights: one F32 scale per output channel.** Static, computed at load.
...
range of each patch honest, which is what a diffusion latent (per-patch
energy varying wildly across the frame) demands.

## The four FC2 strategies — the design space, mapped

The last projection of each MLP (FC2, 14336 → 5376) is the interesting case
...
| `int8-fc2` | per-output-channel | full-width TensorOps int8 product | +3.1% |
...
**quality anchors attached to each**: SSIM 0.919 at 512-class and 0.828 at
...
The 2.6% row-fc2 speedup over plain fc2 (19.32 vs 19.85 s) matters less than
...
3. **A/B at scale**: the SSIM numbers and the L2 numbers of 14h are computed
