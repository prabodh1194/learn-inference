...
The objective here is to cut attention cost by running it over a quarter of
the rows — and to price the output change with a number.

The denoise loop's grid is ~2,835 rows at 512-class (the 512×512 video geometry). Token reduction pools the
...
(relative error vs. the reference latent — the denoised frame the un-optimized path produces) rises to **5.56% at 512-class and
15.14% at 864-class** (the 864×864 video geometry) — the measured price of the
speedup — and the speedup is **28.3%** (39.13 s → 28.06 s at
768p; (39.13 − 28.06) ÷ 39.13 = 28.3%), or 16.69 s → 12.60 s combined with int8.
...
(color fringing at edges, an artifact) with denoise steps ≥ 25** (the step count where the ringing starts to show) — so those two settings (thinning 40+ and reusing the 3 middle layers) are not combined. That is
...
The objective here is to skip whole blocks based on the model's own gate
scores — and to document exactly where the heuristic breaks.

Thinning skips whole blocks. The selection is by **gate rank**: the AdaLN
(adaptive layer norm) gates (14d) are per-block scalars measuring how much each block's output
...
instead of the ideal: blocks 40+ thinnable (at 512-class — the blocks whose
gates are consistently smallest, so the least is lost), block 1 never. The lesson: **gate-ranked heuristics inherit the gate's blind spot**,
and a per-block scalar cannot see structure. The oracle (below) is what
caught it.

## Velocity reuse: extrapolation, clamped

The objective here is to run fewer denoise steps by reusing a velocity from an
earlier step, extrapolated.
...
extrapolation is **clamped to [−2, 2]** — the author's chosen amplitude cap (the code enforces it). The clamp is
...
variational autoencoder's, the decoder network's — domain is bounded); clamping caps the damage at a known amplitude. Every
...
The objective here is to reuse computed sub-expressions across blocks —
algebraically, not by skipping work.
...
The objective here is to give every approximation a reference it is measured
against — so "it changed" becomes a number.
...
??? question "What is an oracle, and why does every approximation need one?"
    ...
    SSIM — the structural-similarity score from 14g — byte-compare where lossless). Without an oracle, an approximation's
...
The objective here is to show how a silent corruption survived every "does it
run" test — and which test caught it.
